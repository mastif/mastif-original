...
$pmh = 'past\s+(medical\s+)?history|pmhx?|pmedhx';
...
$alg = 'allergies|allergy|all|adverse\s+drug\s+reactions?';
$fh = '(?:family\s+(?:(?:medical|social)\s+)?history|fhx?|fam\s+hx)';
$sh = 'social\s+history|social\s+hx|shx?|soc\s*hx|habits';

$objective = 'objective|o\b';
...
$hosp = 'hosp(ital)?\s+course(?:\s+and\s+treatment)?';
$ed = '(?:ed\s+course)|(?:emergency\s+department\s+course)|(?:in\s+(?:the\s+)?ed|\bed)';
$evnt = 'events';

$diag = '(?:cause\s+of\s+death\s+and\s+)?((principal|principle|primary|associated|secondary|additional|other)\s+)?(((pre-)?admission|admit(ting)?|discharge|(pre)?operative)\s+)?(problems\s+and\s+)?diagnos[ei]s';
...
$prob = '(?:(?:patient\s+)?problem\s+list)|(?:significant\s+problems)';
...
$rh_temp_mod = '\s+(?:(?:(?:UP)?ON|AS|AT|\@|FOR|(?:PRIOR\s+TO))\s+(?:(?:(?:THE|THAT)\s+)?(?:TIME|DAY)\s+OF\s+)?(?:ADMISSION|ADMIT|DISCHARGE|TRANSFER|COMPLICATIONS?|HOME|OUTPATIENT))';
$lh_temp_mod = '(?:(?:pre-)?(?:admit(?:ting)?|admission)|current|discharge|follow[\s+\-]?up|home|interim|initial|postpartum|(?:pre|post)op(?:erative)?)\s+';
...
$adate = '(?:(?:admission\s+date)|(?:date\s+of\s+admission))';
$ddate = 'discharge\s+date|date\s+of\s+discharge';
$edate = 'evaluation\s+date';
$dob = '(?:(?:date\s+of\s+birth)|DOB)';
$age = 'age';
$sex = 'sex';
$ddetails = 'discharge\s+patient\s+on';
$rptsts = 'report\s+status';
$attmd = 'attending(?:\s+physician)?';
$srv = 'service';
...
#       $line =~  s/(\d{9,9}\s+\|.+)/\<section ID=\'22\' type=\'HEADER\'\>\<text\>$1\<\/text\>\n\<\/section\>/;
    ...
    elsif  ($line =~m/^\*+INSTITUTION/) {    # Header in UPMC deidientified progress notes
        $line =~ s/(^\*+INSTITUTION)/\<section ID=\'22\' type=\'HEADER\'\>\<title\>\<\/title\>\<text\>$1\<\/text\>/;
     }
    else  {
        unless ($line =~m/^diagnos/i) {
        $line =~ s/(^.)/\<section ID=\'22\' type=\'HEADER\'\>\<title\>\<\/title\>\<text\>$1\<\/text\>/;
        }
    }
...
# Find headings based on key words and phrases
    ...
    if ($line =~ m/$ddate/i) {
      $line =~ s/(($ddate)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'15\' type='DISCHARGE DATE'\>\<title\>$1\<\/title\>\<text\>/ig;

    }

    if ($line =~ m/$edate/i) {
      $line =~ s/(($edate)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'\' type='EVALUATION DATE'\>\<title\>$1\<\/title\>\<text\>/ig;
    ...
    if ($line =~ m/($med2)/i) {
        ...
        $line =~ s/(^($med2)($rh_temp_mod)?($BE|$INCLUDE)?\s*\:)/\<\/text\>\<\/section\>\<section ID=\'28\' type='MEDICATION'\>\<title\>$1\<\/title\>\<text\>/i;
    }
...
#       }
   ...
   if ($line =~ m/$alg/i) {
      $line =~ s/(($alg)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'01\' type='ALLERGY'\>\<title\>$1\<\/title\>\<text\>/ig;
        }
...
#    }
    ...
    if ($line =~ m/$asp/i) {
      $line =~ s/(($lh_temp_mod|$lh_org_mod)?($asp)($rh_temp_mod)?\s*\:)/\<\/text\>\<\/section\>\<section ID=\'04\' type='ASSESSMENT\/PLAN'\>\<title\>$1\<\/title\>\<text\>/ig;
       ...
       if ($line =~ m/$pln/i)  {
          unless ((line =~ m/dspl/i)|| ($line =~ m/$tdpln/i)) {
            $line =~ s/(($lh_temp_mod)?($pln)(?:\s*\:[^\d]|\s*\*))/\<\/text\>\<\/section\>\<section ID=\'33\' type='PLAN'\>\<title\>$1\<\/title\>\<text\>/ig;
          }
       }
    }
    if ($line =~ m/$dspl/i) {
...
#    }
    ...
    if ($line =~ m/$cons/i) {
      $line =~ s/(($cons)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'12\' type='CONSULTANTS'\>\<title\>$1\<\/title\>\<text\>/i;
    }
...
#The followign logic  assumes section labels are on different lines of the input.  This should generally be true but may nto always be. 
...
#      if ($line =~ m/$adate/i) {
#        $line =~ s/(($adate)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'48\' type='ADMISSION DATE'\>\<title\>$1\<\/title\>\<text\>/ig;
#      }
#    }
...
#
     $line =~ s/((?:\s(?:[1]?[0-2]|\d)\.\s+)?($inf)(\s+exam)?\s*\:)/\<\/text\>\<\/subsection\>\<subsection type='Infectious Disease'\>\<title\>$1\<\/title\>\<text\>/i;
#
