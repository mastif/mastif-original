...
$diag = '(?:cause\s+of\s+death\s+and\s+)?((?:final|principal|principle|primary|associated|secondary|additional|other)\s+)?(((?:pre-)?admission|admit(?:ting)?|discharge|(?:pre)?operative)\s+)?(?:problems\s+and\s+)?diagnos[ei]s';
...
$prob = '(?:(?:patient\s+)?problem\s+list)|(?:significant\s+problems)|(?:problems\s+and\s+management\s+are\s+as\s+follows) ';
...
$rh_temp_mod = '\s+(?:(?:(?:UP)?ON|AS|AT|\@|FOR|(?:PRIOR\s+TO)|(?:DURING))\s+(?:(?:(?:THE|THAT|THIS)\s+)?(?:TIME|DAY)\s+OF\s+)?(?:ADMISSION|ADMIT|DEATH|DISCHARGE|TRANSFER|COMPLICATIONS?|HOME|HOSPITALIZATION|OUTPATIENT))';
...
$section_intro = '(?:(?:I\s+(?:personally\s+)?(?:examined|interviewed|saw|managed))|(?:(?:overnight|subjectively)\s+(?:\,\s+)?)?(?:the\s+patient)|Mrs?\.\s+\S+\s+is|(?:I\s+am\s+(?:currently?)(?:managing|seeing))|(?:I\s+was\s+asked))';
    ...
    if ($line =~ m/($hpi1\s+and\s+$reashosp)/i) {
      $line =~ s/(($lh_org_mod)?($hpi1)\s+and\s+($reashosp)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'\' type='HISTORY OF PRESENT ILLNESS\/REASON FOR HOSPITALIZATION'\>\<title\>$1\<\/title\>\<text\>/ig;
        ...
        if ($line =~ m/$hpi1\s*\:/i) {
          ...
          $line =~ s/(^($hpi3)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'23\' type='HISTORY OF PRESENT ILLNESS'\>\<title\>$1\<\/title\>\<text\>/im;
        }
...
#Identify Progress Note sections without headings:

    if ($line =~ m/$section_intro/i) {     
        $line =~ s/(\<section\s+ID\=\'\d*\'\s+type\=\'EVALUATION\s+DATE\'\>\<title\>(?:[^\<])+\<\/title\>\<text\>\n[^<]+)(\n)($section_intro)/$1\<\/text\>\<\/section\>$2\<section ID=\'\' type='PROGRESS SUMMARY'\>\<title\>\<\/title\>\<text\>$3/i;
    }
...
#    }
   ...
   # ADMISSION DATE outside of header in 2010 i2b2 documents (Partners and Beth Israel)
      ...
      if ($line =~ m/^$adate/i) {
        $line =~ s/((^$adate)\s*\:)/\<section ID=\'48\' type='ADMISSION DATE'\>\<title\>$1\<\/title\>\<text\>/i;
      }
      elsif ($line =~ m/$adate/i) {
        $line =~ s/(($adate)\s*\:)/\<\/text\>\<\/section\>\<section ID=\'48\' type='ADMISSION DATE'\>\<title\>$1\<\/title\>\<text\>/ig;
      }
